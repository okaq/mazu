...
class Font {
	...
	init() {
		...
		sce2.ren();
		sce2.cam();
		sce2.scene(this.keys);
		sce2.line(this.keys, this.geo);
		sce2.pop();
	...
	frame() {
		...
		sce2.e.render(sce2.d[k0].b, sce2.c.c);
		this.tick++;
	}
}
...
const sce2 = {
	...
	low(k0, g0) {
		if (g0 == undefined) { console.log(k0); return; }
		let r0 = [];
		for (let i = 0; i < g0.length; i++) {
